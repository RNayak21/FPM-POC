    ...
    return ControllerExtension.extend('demo.com.demofpm.ext.controller.CustomObjectPageController', {
        ...
        override: {
            ...
            onInit: function () {
                ...
                // oModel.bindContext("/getProcessFlowData()")
                //     .requestObject()
                //     .then(oData => {
                //         const oJsonModel = new sap.ui.model.json.JSONModel(oData);
                //         this.getView().setModel(oJsonModel, "data3");  
                //     })
                //     .catch(err => {
                //         console.error("Error loading process flow data", err);
                //     });
            ...
            onAfterRendering: function () {
                ...
                if (!oModel) {
                  console.error("Default model not found!");
                  return;
                }
             
                oModel.bindContext("/getProcessFlowData()") // CDS function call
                  .requestObject()
                  .then(oData => {
                    if (!oData) {
                      console.error("No data returned from getProcessFlowData()");
                      return;
                    }
             
                    const oJsonModel = new sap.ui.model.json.JSONModel(oData);
                    this.getView().setModel(oJsonModel, "data3"); // Set as named model
                  })
                  .catch(err => {
                    console.error("Failed to load process flow data", err);
                  });
            },
